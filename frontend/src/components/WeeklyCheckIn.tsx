  ...
  return (
    <Card className="flex min-h-[26rem] flex-col">
      <div className="flex items-start justify-between gap-3">
        <div>
          <div className="text-[10px] font-semibold uppercase text-[var(--app-muted)]">Weekly check-in</div>
          <div className="mt-1 text-base font-semibold text-[var(--app-ink)]">Shape the next week</div>
        </div>
        <Pill tone={saved?.updatedAt ? 'good' : 'neutral'}>{saved?.updatedAt ? 'saved' : 'open'}</Pill>
      </div>

      <label className="mt-5 block">
        <span className="mb-1.5 block text-[11px] font-semibold text-[var(--app-ink)]">What felt strong?</span>
        <textarea
          value={win}
          onChange={(event) => setWin(event.target.value)}
          rows={3}
          placeholder="A lift, run, habit, or routine that worked"
          className="w-full resize-none radius-control bg-[var(--app-inset)] px-3 py-2.5 text-[13px] leading-relaxed text-[var(--app-ink)] outline-none ring-1 ring-inset ring-[var(--app-line)] placeholder:text-[var(--app-muted)] focus:ring-accent/60"
        />
      </label>

      <label className="mt-3 block">
        <span className="mb-1.5 block text-[11px] font-semibold text-[var(--app-ink)]">What got in the way?</span>
        <textarea
          value={friction}
          onChange={(event) => setFriction(event.target.value)}
          rows={3}
          placeholder="Schedule, recovery, equipment, or anything else"
          className="w-full resize-none radius-control bg-[var(--app-inset)] px-3 py-2.5 text-[13px] leading-relaxed text-[var(--app-ink)] outline-none ring-1 ring-inset ring-[var(--app-line)] placeholder:text-[var(--app-muted)] focus:ring-alert/60"
        />
      </label>

      <div className="mt-3">
        <div className="mb-1.5 text-[11px] font-semibold text-[var(--app-ink)]">Next week</div>
        <div className="grid grid-cols-3 gap-2" role="group" aria-label="Next week intent">
          {INTENTS.map((option) => (
            <button
              ...
              className={`min-h-10 radius-control text-[12px] font-semibold transition-colors ${
                intent === option.value
