...
def fallback_coach_chat(question: str, context: dict[str, Any]) -> str:
    ...
    if training_question and isinstance(activity, dict):
        ...
        if isinstance(today, dict):
            ...
            sleep_confidence = today.get("sleepScoreConfidence")
            if isinstance(sleep_score, (int, float)):
                detail = f"Sleep score: {round(sleep_score)}/100"
                if isinstance(sleep_confidence, str) and sleep_confidence:
                    detail += f" ({sleep_confidence} confidence)"
                parts.append(detail + ".")
            ...
            "messages": [
                {
                    ...
                    "content": (
                        ...
                        "activity.today.sleepScore is a transparent 0-100 sleep signal. Use it alongside its "
                        "confidence; incomplete scores are not evidence of poor recovery. "
                        ...
                        "If food.metabolism is present, you may cite measured maintenance calories (TDEE) and what "
                        "the target implies, but treat it as an estimate from logged data with its stated confidence, "
                        "not a fixed truth, and never push intake below a sensible floor. "
...
# Micronutrients the parser may return, keyed by `<name><Unit>`. Kept to a
...
def _micros(value: Any) -> dict[str, float] | None:
    """Keep only known micronutrient keys with sane numeric values."""
    if not isinstance(value, dict):
        return None
    out: dict[str, float] = {}
    for key in MICRO_KEYS:
        coerced = _macro(value.get(key), 100000)
